...
router.get("/view", eAdmin, (req, res) => {Movimentacao.find({ Push:'Não' }).sort({Push:-1}).then((movimentacao) => {
    res.render("admin/push/view", {movimentacao: movimentacao})}).catch((err) => {
    req.flash("error_msg", "houve um erro ao listar as movimentações")
    res.redirect("/admin/home")})})

// Editar Push:

router.get("/edit/:id", eAdmin2, (req, res) => {Movimentacao.findOne({_id:req.params.id}).then((movimentacao) => {res.render("admin/push/update", {movimentacao: movimentacao})}).catch((err) => {
