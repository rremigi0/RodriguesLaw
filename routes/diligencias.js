...
router.post("/add", eAdmin2, (req, res) => {var erros = []
    ...
    if(erros.length > 0) {res.render("admin/home/main", {erros: erros})}else {
        ...
        res.redirect("/diligencia/pending")}).catch((err) => {
        req.flash("error_msg", "Houve um erro ao cadastrar a Diligência, tente novamente!")
        res.redirect("/diligencia/main")})}})

//Rota Para Visualizar Diligências (R):

    // Rota Principal:
        router.get('/main', eAdmin, (req, res) => {res.render("admin/processos/diligencias/view/pending")})                  

    // Rota de Pendentes:       

        router.get("/pending", eAdmin, async (req, res) => {
            await Diligencia.find( { Status:'Pendente' } ).sort({Prazo:1}).then((diligencias) => {
            res.render("admin/processos/diligencias/view/pending", {diligencias: diligencias})}).catch((err) => {req.flash("error_msg", "houve um erro ao listar as diligencias")
        //res.json(prazos)
            res.redirect("/main")})})
    
    // Rota de Validação: 

        router.get("/validation", eAdmin, (req, res) => {Diligencia.find( { Status:'Validacao' } ).sort({Prazo:1}).then((diligencias) => {res.render("admin/processos/diligencias/view/validation", {diligencias: diligencias})    
        }).catch((err) => {req.flash("error_msg", "houve um erro ao listar as diligencias")
        res.redirect("/main")})})

    // Rota de Erros:

        router.get("/errors", eAdmin, (req, res) => {Diligencia.find( { Status:'Error' } ).sort({Prazo:1}).then((diligencias) => {res.render("admin/processos/diligencias/view/errors", {diligencias: diligencias})    
        }).catch((err) => {req.flash("error_msg", "houve um erro ao listar as diligencias")
        res.redirect("/main")})})
...
router.get("/edit/:id", eAdmin, (req, res) => {Diligencia.findOne({_id:req.params.id}).then((diligencia) => {res.render("admin/processos/diligencias/CRUD/edit", {diligencia: diligencia})}).catch((err) => {
...
router.get("/atualizar/:id", eAdmin2, (req, res) => {Diligencia.findOne({_id:req.params.id}).then((diligencia) => {res.render("admin/processos/diligencias/CRUD/update", {diligencia: diligencia})}).catch((err) => {
